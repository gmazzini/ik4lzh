...
while(!feof($hh)){
  ...
  $myid=$band."-".$parts[2]."-".substr($parts[10+$qq],0,2);
  if(!isset($mult[$myid])){
    $mult[$myid]=1;
    if(!isset($amult[$mytt]))$amult[$mytt]=1;
    else $amult[$mytt]++;
  }
  if(isset($parts[11+$qq])){
    $myid=$band."-".$parts[2]."!".(int)$parts[11+$qq];
    if(!isset($mult[$myid])){
      $mult[$myid]=1;
      if(!isset($amult[$mytt]))$amult[$mytt]=1;
      else $amult[$mytt]++;
